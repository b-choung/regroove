...
카드 안에 상세 열기 버튼, 원문 링크, 상태 Select가 있어서 카드 전체를 드래그 대상으로 만들면 클릭이 삼켜집니다. 전용 손잡이에만 드래그 리스너를 붙이고, `PointerSensor`에 8px 활성화 거리를 둬서 이중으로 막았습니다. 손잡이는 버튼이라 `KeyboardSensor`로 키보드 이동도 됩니다. dnd-kit 기본 안내 문구는 영어라 "관심 컬럼에 놓았습니다" 식의 한국어 안내로 교체했습니다.
...
- **2주차**: dnd-kit 드래그앤드롭(키보드 이동 포함), 낙관적 업데이트와 롤백, 카드 상세 다이얼로그와 메모 CRUD. 카드의 상태 Select는 터치·키보드용 대체 수단으로 남겨 뒀습니다.
- **3주차**: URL 파싱 폴백 체인(JSON-LD/OG → Claude → 수동 입력), 파싱 결과 안내, 스킬 프로필 저장과 매칭률 표시.
- **4주차**: 대시보드(요약·단계 전환율·마감 알림·상태별 분포), 카드 마감일 D-day 강조, 로그인 후 화면을 라우트 그룹 레이아웃으로 정리, 배포 절차 문서화.
