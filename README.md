...
매일 여는 도구라서 화려함보다 조용함을 목표로 했습니다. 토큰은 [app/globals.css](app/globals.css)의 `@theme` 한 곳에 있습니다 — Tailwind v4는 `tailwind.config.ts` 없이 CSS에서 테마를 정의하므로 여기가 단일 출처입니다.

**컬러 — 세 계열만.** 중립(캔버스·표면·테두리·텍스트) + 포인트 1개 + 경고 1개.

| 역할                      | 토큰                                       | 값                                |
| ------------------------- | ------------------------------------------ | --------------------------------- |
| 캔버스 / 표면 / 표면 보조 | `background` `card` `muted`                | `#f9fafb` / `#ffffff` / `#f3f4f7` |
| 테두리                    | `border`                                   | `#e2e4e8`                         |
| 본문 / 보조 텍스트        | `foreground` `muted-foreground`            | `#1c1f25` / `#686c74`             |
| 포인트                    | `primary` `primary-hover` `primary-subtle` | `#3e61c1` / `#3050aa` / `#eaf0fe` |
| 경고·삭제                 | `destructive` `destructive-subtle`         | `#c92f33` / `#feebe9`             |

무채색도 순수 흑백이 아니라 포인트 컬러와 같은 264° 쪽으로 아주 살짝 기울였습니다(chroma 0.002~0.014). 흰 배경 위 `primary` 대비는 5.7:1로 흰 글씨가 WCAG AA를 넘깁니다.

포인트 컬러를 **쓰는 곳만** 정해 뒀습니다: 주 CTA, 활성 탭, 대시보드 막대, 드롭 대상 하이라이트, 포커스 링. 카드 테두리·컬럼 헤더·본문에는 쓰지 않습니다. 리팩토링 전에는 `--primary`가 무채색이라 CTA와 막대가 전부 회색이었고, 대신 스킬 배지와 파싱 안내가 emerald/amber를 각자 끌어와 팔레트가 흩어져 있었습니다.

**타이포 — 4단.** 크기를 늘리는 대신 굵기로 위계를 만듭니다.

| 토큰           | 크기               | 용도                                  |
| -------------- | ------------------ | ------------------------------------- |
| `text-display` | 24px / 600         | 대시보드 숫자 (화면에서 가장 큰 글자) |
| `text-heading` | 20px / 600         | 페이지 제목                           |
| `text-body`    | 14px / 400·500·600 | 본문, 카드 공고명, 섹션 제목(600)     |
| `text-caption` | 12px               | 회사명, 마감일, 힌트, 카운트          |

가장 큰 글자를 페이지 제목이 아니라 대시보드 숫자에 줬습니다. 매번 확인하는 건 "대시보드"라는 제목이 아니라 그 아래 숫자입니다. 이름이 용도를 말하게 해서, 12px 캡션이 파일마다 `text-xs`로 흩어지지 않게 했습니다.

**여백·테두리·그림자.**

```
spacing   4px 그리드. 페이지 24px(모바일)·32px(lg+), 섹션 간 28px,
          칸반 카드 14px, 대시보드 카드·다이얼로그 20px
radius    md 6px(버튼·입력) / lg 8px(카드·팝오버) / xl 12px(다이얼로그)
          배지와 막대만 pill
border    1px, 한 표면에 하나. ring과 border를 겹쳐 그리지 않는다
shadow    한 단계(shadow-overlay). 다이얼로그·팝오버·드래그 중 카드에만.
          평상시 표면은 테두리로만 구분한다
```

칸반 컬럼은 배경도 테두리도 없습니다. 컬럼에 배경을 깔면 그 안의 카드와 박스가 두 겹이 되고, 6개 컬럼이 회색 블록 6개로 읽혔습니다. 떠 있는 표면은 카드 하나로 두고, 컬럼은 드롭 대상일 때만 `primary-subtle` 배경이 깔립니다.

**폰트.** Pretendard 가변 폰트 하나를 직접 호스팅합니다([app/fonts/](app/fonts/), OFL 1.1). 이전 Geist는 라틴 전용이라 한글이 OS 기본 폰트로 떨어져, 같은 줄의 "토스"와 "D-7"이 서로 다른 폰트로 보였습니다. 파일이 2MB라 `preload`는 끄고 `display: swap`으로 뒀습니다 — 첫 방문에서 텍스트 표시를 폰트가 붙잡지 않고, `next/font/local`이 자동으로 만들어 주는 폴백 메트릭 덕에 교체 시점의 레이아웃 이동도 없습니다.

다크 모드 토큰은 같은 규칙으로 맞춰 뒀지만 `ThemeProvider`가 없어 아직 켜지지 않습니다.
